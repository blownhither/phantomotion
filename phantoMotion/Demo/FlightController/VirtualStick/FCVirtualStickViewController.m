...
@property (weak, nonatomic) IBOutlet UILabel *leapmotionLabel;
...
- (void)setAllAssetsEnable:(BOOL)enable {
//    self.joystickRight.hidden = YES;
//    ShowResult(@"in setAllAssetsEnable");
//    [self.joystickRight setHidden:YES];
    [self.joystickLeft setUserInteractionEnabled:enable];
    [self.joystickRight setUserInteractionEnabled:enable];
    [self.coordinateSys setUserInteractionEnabled:enable];
    [self.enableVirtualStickButton setUserInteractionEnabled:enable];
    [self.simulatorButton setUserInteractionEnabled:enable];
//    ShowResult(@"out setAllAssetsEnable");
}

- (void) listenLeapmotionServe {
//    ShowResult(@"listening leapmotion!!!!!!!!");
...
//    while (1){
    ...
    self.logLabel3.text = str;
    if (str && [str length]){
        NSArray *pos = [str componentsSeparatedByString:@":"];
        self.leapmotionLabel.text = [NSString stringWithFormat:@"%lu", [pos count]];
        //in-data order: roll pitch yaw throttle
        @try {
            if ([pos count] == 4) {
                ...
                mXVelocity = [pos[0] floatValue] * 15;
                mYVelocity = [pos[1] floatValue] * 15;
                mYaw = [pos[2] floatValue] * 30;
                mThrottle = [pos[3] floatValue] * 2;
                if (fabsf(mXVelocity) < 15 &&
                    fabsf(mYVelocity) < 15 &&
                    fabsf(mYaw) < 20 &&
                    fabsf(mThrottle) < 2) {
                    [self updateJoystick];
            ...
            self.logLabel0.text = [NSString stringWithFormat:@"%lu", [pos count]];
            self.logLabel1.text = exception.name;
            self.logLabel2.text = str;
...
//    self.leapmotionLabel.text = @"update!";
    ctrlData.pitch = mYVelocity;
    ctrlData.roll = mXVelocity;
    ctrlData.yaw = mYaw;
    ctrlData.verticalThrottle = mThrottle;
//    self.logLabel.text = [NSString stringWithFormat: @"%f   %f   %f   %f", ctrlData.pitch, ctrlData.roll, ctrlData.yaw, ctrlData.verticalThrottle];
//    NSLog(@"%f   %f   %f   %f", ctrlData.pitch, ctrlData.roll, ctrlData.yaw, ctrlData.verticalThrottle);
    DJIFlightController* fc = [DemoComponentHelper fetchFlightController];
